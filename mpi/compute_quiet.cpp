...
void pre_compute(std::vector<std::vector<int64_t>> &vect, int64_t start_2,
                 int64_t start, int64_t max_val) {
    int64_t lastPrint;
    std::chrono::steady_clock::time_point startBar;
    std::chrono::steady_clock::time_point stopBar;
    int64_t start_min = std::ceil(std::sqrt(start_2 / 2));
    for (int64_t i = start_min; i < start; i++) {
        int64_t j_min = std::ceil(std::sqrt(start_2 - i * i));
        int64_t j_lim = std::ceil(std::sqrt(max_val - i * i));
...
void compute(std::vector<std::vector<int64_t>> &vect, int64_t start,
             int64_t start_2, int64_t max_lim, int64_t max_val) {
    int64_t lastPrint;
    std::chrono::steady_clock::time_point startBar;
    std::chrono::steady_clock::time_point stopBar;
    for (int64_t i = start; i < max_lim; i++) {
        int64_t j_lim = std::ceil(std::sqrt(max_val - i * i));
        int64_t j_max = std::min(i, j_lim);
...
void post_compute(std::ostream &stream, std::vector<std::vector<int64_t>> &vect,
                  int64_t vect_len) {
    int64_t lastPrint;
    std::chrono::steady_clock::time_point startBar;
    std::chrono::steady_clock::time_point stopBar;
...
void main_compute(std::ostream &stream, int64_t start, int64_t stop) {
    stream << start << " " << stop << "\n";
    int64_t start_2 = start * start;
    int64_t stop_2 = stop * stop;
    int64_t vect_len = stop_2 - start_2;
    std::vector<std::vector<int64_t>> vect(vect_len);
    pre_compute(vect, start_2, start, stop_2);
    compute(vect, start, start_2, stop, stop_2);
    post_compute(stream, vect, vect_len);
}
}  // namespace compute_quiet